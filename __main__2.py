...
@app.route("/api/descargar/<file_id>", methods=['GET', 'POST'])
def api_descargar(file_id):
        ruta = 'fotos/'
	archivos = ""
	for file in os.listdir(ruta):
	...
	if archivos:
		nombreArchivo = "fotos_" + file_id + ".tar.gz"
		try:
                        text = getstatusoutput("tar -zcvf " + ruta + nombreArchivo + archivos)		
			#return send_from_directory(directory=ruta, filename=nombreArchivo, as_attachment=True)
...
def api_descargarlogs():
       return send_file("/var/log/supervisor/supervisor.err.log", as_attachment=True)	
